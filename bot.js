...
const DEATHS_GAME = 'Rayman';
const DEATHS_FILE = path.join(__dirname, 'rayman_deaths.json');
const songQueue = [];
...
class DeathCounter {
    ...
    loadDeaths() { try { if (fs.existsSync(DEATHS_FILE)) { this.data = JSON.parse(fs.readFileSync(DEATHS_FILE, 'utf8')); } else { this.data = { deaths: 0, lastUpdated: new Date().toISOString(), game: DEATHS_GAME }; this.saveDeaths(); } } catch (e) { console.error('Error cargando muertes:', e); this.data = { deaths: 0, lastUpdated: new Date().toISOString(), game: DEATHS_GAME }; } }
